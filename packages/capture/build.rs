...
fn main() {
    ...
    // Safety: Copy dist to a staging area to ensure we don't zip a folder being modified
    let stage_dir = out_dir.join("dist_stage");
    if stage_dir.exists() { let _ = fs::remove_dir_all(&stage_dir); }
    fs::create_dir_all(&stage_dir).expect("Failed to create stage dir");

    // Copy dist to stage
    if dist_path.exists() {
        copy_dir_recursive(&dist_path, &stage_dir).expect("Failed to stage dist folder");
        compress_dist(&stage_dir, &output_zip);
    } else if env::var("PROFILE").unwrap() != "release" {
        // Fallback for debug if dist missing (logic above should catch this but double check)
        create_dummy_zip(&output_zip);
    } else {
        panic!("dist folder missing for Release build!");
    }
...
fn max_mtime(path: &Path) -> Option<SystemTime> {
    WalkDir::new(path).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file())
        ...
        .filter_map(|m| m.modified().ok()).max()
}

fn min_mtime(path: &Path) -> Option<SystemTime> {
    WalkDir::new(path).into_iter().filter_map(|e| e.ok()).filter(|e| e.file_type().is_file())
        .filter_map(|e| e.metadata().ok())
        .filter_map(|m| m.modified().ok()).min()
}
